...
exports.createCourse = async (req,res)=>{
     ...
     try {
		...
		if (!categoryDetails) {
		 ...
		 const requirements = instructions.split(",");
		...
		const val = await Category.findByIdAndUpdate(
			{ _id: category },
			{
				$push: {
					courses: newCourse._id,
...
exports.getAllCourses = async (req,res) => {
	try {
		...
		const allCourses = await Course.find({}, {new : true}).populate('instructor').populate('category').exec();
