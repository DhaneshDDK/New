...
const VideoDetailsSidebar = ({setReviewModal}) => {
  ...
  const [isActive, setIsActive] = useState([sectionId])
      ...
      <div className="flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px] border-r-richblack-700 bg-richblack-800">
        <div className="mx-5 flex flex-col items-start justify-between gap-2 gap-y-4 border-b border-richblack-600 py-5 text-lg font-bold text-richblack-25">
          ...
          <div className="flex flex-col">
            <p className='text-2xl'>{courseEntireData?.courseName}</p>
            <p className="text-sm my-1 font-semibold text-richblack-500">
                ...
                <div className="transition-[height] duration-500 ease-in-out">
                  {course.subSection.map((topic, i) => (
                    <div
                      className={`flex gap-3  px-5 py-2 ${
                        videoBarActive === topic._id
                          ? "bg-yellow-50 font-semibold text-richblack-800"
                    ...
                    >
                      <input
                        type="checkbox"
                        className="border-gray-300 h-4 w-4 cursor-pointer rounded accent-caribbeangreen-100"
